...
struct line_vertex_point
{
	unsigned char r, g, b, a;
	float x, y, z;
};

line_vertex_point g_lineVertices[] =
{
...
line_vertex_point g_lineVertices_realsense[] =
{
	{ 255, 0, 0, 255, 0.0f, 0.0f, 0.0f },
	{ 255, 0, 0, 255, 0.2f, 0.0f, 0.0f },
	{ 255, 0, 0, 255, 0.0f, 0.0f, 0.0f },
...
void show_points(GLFWwindow* window, GlobalMap s, Camera camera, Camera kinect){
	...
	if (!glfwWindowShouldClose(window))
	{
		...
		float theta_x_realsense = atan2(kinect.get_rotation()[7], kinect.get_rotation()[8]);
		float theta_y_realsense = atan2(-kinect.get_rotation()[6], sqrt(pow(kinect.get_rotation()[7], 2) + pow(kinect.get_rotation()[8], 2)));
		float theta_z_realsense = atan2(kinect.get_rotation()[3], kinect.get_rotation()[0]);
		...
		float pos_x_realsense = kinect.get_position()[0];
		float pos_y_realsense = kinect.get_position()[1];
		float pos_z_realsense = kinect.get_position()[2];
...
void show_location(GLFWwindow* window, int n, GLfloat *vertices, GLfloat *colors, Camera camera, Camera kinect, vector<Camera> kinect_pos){
	...
	if (!glfwWindowShouldClose(window))
	{
		...
		float theta_x_realsense = atan2(kinect.get_rotation()[6], kinect.get_rotation()[0]);
		float theta_z_realsense = atan2(kinect.get_rotation()[5], kinect.get_rotation()[4]);
		float theta_y_realsense = asin(kinect.get_rotation()[3]);
		...
		for (int i = 0; i < kinect_pos.size(); i++){
			glLineWidth(0.01f);
			glColor4f(0.0f, 1.0f, 0.0f, 1.0f);
			glBegin(GL_POINTS);
			glVertex3f(kinect_pos[i].get_position()[0], kinect_pos[i].get_position()[1], kinect_pos[i].get_position()[2]);
			glEnd();
		}
...
int terminate_point_renderer_3d(GLFWwindow* window)
{
