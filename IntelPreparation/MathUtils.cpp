...
float * MathUtils::get_rotation_matrix_glm(float angle_x, float angle_y, float angle_z){
	...
	float rad_angle_x = (float)(-1.0f * ((angle_x*PI) / 180.0f));
	float rad_angle_y = (float)(-1.0f * ((angle_y*PI) / 180.0f));
	float rad_angle_z = (float)(-1.0f * ((angle_z*PI) / 180.0f));
	glm::mat3 rotation_matrix = glm::mat3();
	glm::mat3 rotation_x(1, 0, 0, 0, cos(rad_angle_x), -sin(rad_angle_x), 0, sin(rad_angle_x), cos(rad_angle_x));
	glm::mat3 rotation_y(cos(rad_angle_y), 0, sin(rad_angle_y), 0, 1, 0, -sin(rad_angle_y), 0, cos(rad_angle_y));
	glm::mat3 rotation_z(cos(rad_angle_z), -sin(rad_angle_z), 0, sin(rad_angle_z), cos(rad_angle_z), 0, 0, 0, 1);
